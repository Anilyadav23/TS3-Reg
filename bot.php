...
// Connect to the TS server and get the server info.
$virt = TeamSpeak3::factory(TS_QUERY);
$serverInfo = $virt->getInfo();

/***************************** UNUSUED KEYS RULE ******************************/
if(RULE_PURGE_KEYS){
    // Go through all keys on the server and see which ones can be purged.
    foreach($virt->privilegeKeyList() as $privKey){
        ...
        $query = mysql_query("SELECT *
                              ...
                              WHERE (`deleted` = '1'
                                     OR `registeredTimestamp` <= '".date("Y-m-d H:i:s", strtotime("-" . RULE_PURGE_KEYS_DAYS . " days"))."')
                                AND `token` = '".$token."'");
        if(mysql_num_rows($query) > 0){
            // Delete from the server.
            //$virt->privilegeKeyDelete($token);
            print "Deleting key " . $token . " as not in local db.<br />";
        }
    }
}

/******************************* CLIENT RULES **********************************/
if(RULE_REG || RULE_IDLE || RULE_AUTO_SA){
    foreach($virt->clientList() as $client)
    {
        ...
        if((strcasecmp(trim($client["client_nickname"]), trim($member->certName)) != 0)){
                    ...
                    //check the aliases
                    $aliases = mysql_query("SELECT alias 
                                                                            FROM alias
                                                                            WHERE cid=".intval($member->memberID));
                    $aliasFound = 0;
                    //loop through to see if the user has an alias that matches their vatsim CID
                    while ($alias = mysql_fetch_object($aliases)){
                            if ((strcasecmp(trim($client["client_nickname"]), trim($alias->alias)) == 0)){
                                    $aliasFound = 1;
                            }
                    }

                    //if there was no alias found, kick the user
                    if ($aliasFound!=1){
